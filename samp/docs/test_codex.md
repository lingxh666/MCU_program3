...
### 0.3 清理方式（已确认：git restore）

执行测试时（不是现在），**每一步 PASS 后统一使用 Git 回滚清理临时测试代码**（不手工删）。下文所有“通过后清理：restore/restore freertos_app.c”均指执行以下命令：
...
你已确认选择 **选项 1**：每一步 PASS 后按 **0.3** 执行 `git restore samp/project/src/freertos_app.c` 清理临时测试代码（不采用手工删除）。
