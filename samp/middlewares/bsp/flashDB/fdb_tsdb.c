...
//    FDB_INFO("to %s", ctime((const time_t * )&to));
    ...
    if (debug_once < 3) {
        debug_once++;
        FDB_INFO("TSDB_ITER_DEBUG: start_addr=0x%08X, from=%lu, to=%lu\n", start_addr, (unsigned long)from, (unsigned long)to);
        FDB_INFO("TSDB_ITER_DEBUG: db_max_size=%lu, db_sec_size=%lu\n", 
                 (unsigned long)db_max_size(db), (unsigned long)db_sec_size(db));
    }
    ...
    do {
        traversed_len += db_sec_size(db);
        // Read sector info - use traversal=true for USING sectors to get correct end_time
        // First read without traversal to check status
        if (read_sector_info(db, sec_addr, &sector, false) != FDB_NO_ERR) {
            if (debug_once <= 3) FDB_INFO("TSDB_ITER_DEBUG: sector 0x%08X read error\n", sec_addr);
            continue;
        }
        // If USING sector, re-read with traversal to get accurate end_time
        if (sector.status == FDB_SECTOR_STORE_USING) {
            read_sector_info(db, sec_addr, &sector, true);
        }
        ...
        sector_count++;
        if (debug_once <= 3) {
            // Print first 5 and last 5 sectors, and any USING/FULL sectors
            if (sector_count <= 5 || sector.status == FDB_SECTOR_STORE_USING || sector.status == FDB_SECTOR_STORE_FULL) {
                FDB_INFO("TSDB_ITER_DEBUG: sector[%lu] addr=0x%08X, status=%d, start_time=%lu, end_time=%lu\n",
                         sector_count, sec_addr, sector.status, (unsigned long)sector.start_time, (unsigned long)sector.end_time);
            }
        }
        
        /* sector has TSL */
        if ((sector.status == FDB_SECTOR_STORE_USING || sector.status == FDB_SECTOR_STORE_FULL)) {
            bool is_using_sector = (sector.status == FDB_SECTOR_STORE_USING);
            if (is_using_sector) {
                /* copy the current using sector status  */
                sector = db->cur_sec;
            }
            // For USING sector, always traverse (end_time may not be updated yet)
            // For FULL sector, check time range
            if ((found_start_tsl)
                    || is_using_sector  // Always check USING sector
                    || (!found_start_tsl &&
                ...
                do {
                    ...
                    if (tsl.status != FDB_TSL_UNUSED) {
                        if ((from <= to && tsl.time >= from && tsl.time <= to)
                            ...
                            // TSL time out of range, but don't exit - continue to next sector
                            // This allows finding records in later sectors
            ...
            // Don't exit on EMPTY sector, continue to next sector (for rollover support)
            // goto __exit;  // REMOVED: This prevents finding records after rollover
            if (debug_once <= 3) {
                FDB_INFO("TSDB_ITER_DEBUG: sector[%lu] addr=0x%08X is EMPTY, continue...\n", sector_count, sec_addr);
            }
        }
        sec_addr = get_sector_addr(db, &sector, traversed_len);
        if (debug_once <= 3 && sec_addr == FAILED_ADDR) {
            FDB_INFO("TSDB_ITER_DEBUG: get_sector_addr returned FAILED_ADDR, traversed_len=%lu\n", traversed_len);
        }
    } while (sec_addr != FAILED_ADDR);

__exit:
    if (debug_once <= 3) {
        FDB_INFO("TSDB_ITER_DEBUG: iteration end, total_sectors=%lu, found_start_tsl=%d, traversed_len=%lu\n", 
                 sector_count, found_start_tsl, traversed_len);
    }
    db_unlock(db);
}
