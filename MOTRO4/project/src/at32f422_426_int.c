...
volatile uint8_t g_1s_update_flag = 0;
...
void TMR6_GLOBAL_IRQHandler(void)
{
  ...
  if(tmr_interrupt_flag_get(TMR6, TMR_OVF_FLAG) != RESET)
  {
    ...
    MotorUpdate(MOTOR_ID_3);
    MotorUpdate(MOTOR_ID_4);
...
/**
  * @brief  this function handles TMR15 handler.
  * @param  none
  * @retval none
  */
void TMR15_GLOBAL_IRQHandler(void)
{
  /* add user code begin TMR15_GLOBAL_IRQ 0 */

  /* add user code end TMR15_GLOBAL_IRQ 0 */

  /* overflow interrupt management */
  if(tmr_interrupt_flag_get(TMR15, TMR_OVF_FLAG) != RESET)
  {
    /* add user code begin TMR15_TMR_OVF_FLAG */
    tmr_flag_clear(TMR15, TMR_OVF_FLAG);
    /* 1Hz秒任务标志 */
    g_1s_update_flag = 1;
    /* add user code end TMR15_TMR_OVF_FLAG */
  }

  /* add user code begin TMR15_GLOBAL_IRQ 1 */
