...
/* SRAM扩展配置宏定义 */
#define EXTEND_SRAM_224K  0xFE  /* 使用224K SRAM */
#define EXTEND_SRAM_96K   0xFF  /* 使用96K SRAM (默认) */

/**
  * @brief  检查并扩展SRAM到224KB
  * @note   此函数必须在SystemInit之前调用（由启动文件调用）
  *         配置生效需要系统复位
  * @param  none
  * @retval none
  */
void extend_sram_check(void)
{
  /* 检查当前SRAM配置是否已经是224K */
  if(((USD->eopb0) & 0xFF) != EXTEND_SRAM_224K)
  {
    /* 解锁FLASH */
    flash_unlock();
    ...
    /* 编程eopb0为0xFE，配置224K SRAM */
    flash_user_system_data_program((uint32_t)&USD->eopb0, EXTEND_SRAM_224K);
